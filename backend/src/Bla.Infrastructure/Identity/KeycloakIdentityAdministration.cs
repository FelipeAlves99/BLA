...
internal sealed class KeycloakIdentityAdministration(HttpClient client, IConfiguration configuration)
    : IIdentityAdministration
{
    public async Task<IdentityRegistrationResult> RegisterAsync(string username, string email, string displayName,
        string password, CancellationToken ct)
    {
        ...
        var secret = configuration["KeycloakRegistration:ClientSecret"];
        if (string.IsNullOrWhiteSpace(secret))
            secret = Environment.GetEnvironmentVariable("KEYCLOAK_REGISTRATION_CLIENT_SECRET");
        if (string.IsNullOrWhiteSpace(secret))
            throw new InvalidOperationException("Keycloak registration client secret is not configured.");
        ...
        using var tokenRequest = new HttpRequestMessage(HttpMethod.Post, $"{authority}/protocol/openid-connect/token")
        {
            Content = new FormUrlEncodedContent([
                new("grant_type", "client_credentials"), new("client_id", "bla-registration-api"),
        ...
        using var createRequest = new HttpRequestMessage(HttpMethod.Post,
            new Uri($"{new Uri(authority).GetLeftPart(UriPartial.Authority)}/admin/realms/{realm}/users"))
        {
        ...
        if (createResponse.StatusCode == HttpStatusCode.Conflict)
            return new IdentityRegistrationResult(null, true, null);
        if (!createResponse.IsSuccessStatusCode)
            return new IdentityRegistrationResult(null, false, "Could not create the account.");
